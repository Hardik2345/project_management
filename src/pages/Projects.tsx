...
export function Projects() {
  ...
  const handleDropdownAction = async (action: string, project: any) => {
    setActiveDropdown(null);
    ...
    switch (action) {
      ...
      case 'duplicate':
        try {
          ...
          const createdProject = await apiClient.createProject(duplicatedProject);
          dispatch({ type: 'ADD_PROJECT', payload: createdProject });
        } catch (error) {
      ...
      case 'archive':
        try {
          const updatedProject = await apiClient.updateProject(project.id, { status: 'cancelled' });
